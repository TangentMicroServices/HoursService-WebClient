...
describe('Service: userService', function () {
  ...
  it('When user logges in succesfully, make sure access token is set.', inject(function($httpBackend, SERVICE_BASE_URI, $window){
    spyOn($window.localStorage, 'setItem');

    $httpBackend.expectPOST(SERVICE_BASE_URI + '/api-token-auth/', fakeLoginRequest).respond(200, fakeLoginSuccessResponse);
  ...
  it('When user logges in succesfully, make sure a post request is made to user service.', inject(function($httpBackend, SERVICE_BASE_URI){
    userService.Login(fakeLoginRequest.username, fakeLoginRequest.password);
  ...
  it('When fetching the current user from the access token, make sure that the current user is set in the rootScope.', inject(function($httpBackend, SERVICE_BASE_URI, $rootScope){
    $httpBackend.expectGET(SERVICE_BASE_URI + '/users/me/').respond(200, fakeUser);
